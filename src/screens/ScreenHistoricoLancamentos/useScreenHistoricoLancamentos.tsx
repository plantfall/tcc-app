...
import {LoteRecebimento, LoteStatus} from '../../@types/Recebimento.types';
...
import {DefaultApiProvider} from '../../service/DefaultApiProvider';
import {LotesDoUsuarioMock} from '../../mocks/Lancamentos.mock';
...
export function useScreenHistoricoLancamentos() {
    ...
    useCallback(() => {
      ...
      const fetchLotes = async () => {
        try {
          ...
          if (lotesDaFila.length == 0) {
            setLoading(true);
          }
          setLancamentos(lotesDaFila);

          const lotesDoUsuarioFromApi = DefaultApiProvider.TEST_MODE
            ? LotesDoUsuarioMock
            : await recService.lotesDoUsuario(token);
